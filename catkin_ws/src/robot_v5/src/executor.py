...
class InteractionThread(threading.Thread):
    ...
    def logic(self):
        ...
        try:
            ...
            if robotStatus == "idle":
                print("\nnow robot status is idle\n")
            
            if robotStatus == "task":
                print("\nnow robot status is task\n")

            if robotStatus == "back":
                print("\nnow robot status is back\n")
...
if __name__ == "__main__":
    ...
    mqtt_thread.start()
    interaction_thread = InteractionThread(robotState=robotState, instructionInfo=instructionInfo, http_client=http_client, programStatus=programStatus, elevatorControl=elevatorControl , ros_pub_goal=ros_pub_goal, stop_event=stop_event)
